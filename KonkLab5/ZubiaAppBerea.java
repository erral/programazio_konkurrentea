...
class KotxeGorria extends Thread {
    ...
    public void run() {
      try {
        while(true) {
            //while (!display.moveRed(zenb));
            while (!erakutsi.mugituGorria(zenb));  // ez dago zubian
            kontrola.sartuGorria();		//eskatzen du zubiaren atzipena
            while (erakutsi.mugituGorria(zenb));   // zubian mugitu
            kontrola.irtenGorria();		//askatzen du zubiaren atzipena
        }
...
class KotxeUrdina extends Thread {
    ...
    public void run() {
      try {
        while(true) {
            //while (!display.moveRed(zenb));
            while (!erakutsi.mugituUrdina(zenb));  // ez dago zubia
            kontrola.sartuUrdina();
            while (erakutsi.mugituUrdina(zenb));   // zubian mugitu
...
class BidezkoZubia extends Zubia {
    private int kGorria = 0;  //kotze gorrien kopurua zubian;
    private int kUrdina = 0;  //kotze urdinen kopurua zubian;
    private int zaiGorria = 0;//sartzeko zai dauden kotze gorrien kopurua
	private int zaiUrdina = 0;//sartzeko zai dauden kotze urdinen kopurua
